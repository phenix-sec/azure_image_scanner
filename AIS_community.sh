...
#MUST CHANGE
imagesfile="images_community.csv"
...
resource_id_column=8 #RESOURCE ID column number from .csv images file (default 8, but can change)
...
regionname=`curl -s -H Metadata:True "http://169.254.169.254/metadata/instance?api-version=2017-04-02&format=json" | jq -r '"\(.compute.location)"' | awk '{print tolower($0)}'`
vmname=`hostname`
...
#parse images file to get Resource Ids for current region
community_images=`cat $imagesfile | grep -i "/locations/${regionname}/" | cut -d "," -f ${resource_id_column} | sed -n -e 's/^.*\(\/CommunityGalleries\)/\1/p' | cut -d "\"" -f 1 | sed -e "s/$/\/Versions\/latest/"`
#get the number of images from imagesfile
size=`echo "$community_images" | wc -l`
...
for ((line=$starting_line;line<=$size;line++)); do
	...
	line_next1=$(( ${line} + 1 ))
	line_next2=$(( ${line} + 2 ))
	image_current=`echo "$community_images" | sed -n ${line}p`
	image_next1=`echo "$community_images" | sed -n ${line_next1}p`
	image_next2=`echo "$community_images" | sed -n ${line_next2}p`
	...
	if [ $line -eq $(($starting_line)) ]; then
		...
		az disk create -g $resourcegroup -n disk${regionname}$line --location $regionname --gallery-image-reference $image_current --zone 1 --security-type Standard --only-show-errors &> /dev/null
		echo -e "Created Disk $line"
		az disk create -g $resourcegroup -n disk${regionname}$line_next1 --location $regionname --gallery-image-reference $image_next1 --zone 1 --security-type Standard --only-show-errors &> /dev/null
		echo -e "Created Disk $line_next1"
		sleep 2
		diskId=$(az disk show -g $resourcegroup -n disk${regionname}$line --query 'id' -o tsv)
	...
	if [ $line -ge $(($starting_line+2)) ]; then
		...
		while [[ `az disk show -n disk${regionname}$line_last2 -g $resourcegroup | jq -r '"\(.diskState)"'` = "Attached" ]]; do sleep 5; echo "WAITING DISK DETACHMENT"; done
		echo -e "\nDELETE disk $line_last2 !!!!!!!!!!"
		az disk delete --name disk${regionname}$line_last2 --resource-group $resourcegroup -y --no-wait true
	fi


	#create line_next2
	echo -e "\nCREATE disk $line_next2 !!!!!!!!!!"
	az disk create -g $resourcegroup -n disk${regionname}$line_next2 --location $regionname --gallery-image-reference $image_next2 --zone 1 --security-type Standard --no-wait --only-show-errors
	...
	if [ $skip_mount -eq 0 ]; then
		...
		az disk show -n disk${regionname}$line -g $resourcegroup | jq -r '"\(.diskState)"'
		while [[ `az disk show -n disk${regionname}$line -g $resourcegroup | jq -r '"\(.diskState)"'` != "Attached" ]]; do 
			sleep 5; echo "DISK NOT YET ATTACHED"; 
			if [[ `az disk show -n disk${regionname}$line -g $resourcegroup | jq -r '"\(.diskState)"'` != "Attached" ]];then 
				diskId=$(az disk show -g $resourcegroup -n disk${regionname}$line --query 'id' -o tsv); 
				az vm disk attach -g $resourcegroup --vm-name $vmname --name $diskId; 
				echo "DISK ATTACHED"; 
			fi; 
		done
		publisher=`echo "$community_images" | sed -n ${line}p | cut -d "/" -f 3 | cut -d "-" -f 1`
		echo -e "\nPUBLISHER name: $publisher"
		#check if previous disk detached
		if [ $line -ge $(($starting_line+1)) ]; then
			while [[ `az disk show -n disk${regionname}$line_last1 -g $resourcegroup | jq -r '"\(.diskState)"'` = "Attached" ]]; do sleep 5; echo "MULTIPLE DISKS ATTACHED"; done
		fi
			...
			while read -r volume <&3; do
				if [ "$(echo "`lsblk -rno "NAME,SIZE" | grep "${volume} " | cut -d " " -f 2 | grep G | cut -d "G" -f 1` > 1.8" | bc)" -eq 1 ] && [ "$(echo "`lsblk -rno "NAME,SIZE" | grep "${volume} " | cut -d " " -f 2 | grep G | cut -d "G" -f 1` < 260" | bc)" -eq 1 ]; then
					...
					if [ "`lsblk -rno NAME,FSTYPE | grep "${volume} " | cut -d " " -f 2`" = "LVM2_member" ];then 
						...
						./azcopy copy --recursive $storage ${containerurl}/${regionname}/${publisher}${sastoken} &>> azurecopy.txt
		...
		if [ $line -ge $(($starting_line)) ]; then
			...
			az vm disk detach -g $resourcegroup --vm-name $vmname -n disk${regionname}$line --force-detach >>logs.txt
		fi
	else
		echo "MOUNT SKIPPED";
	fi
	...
	if [[ `az disk show -n disk${regionname}$line_next1 -g $resourcegroup | jq -r '"\(.diskState)"'` != "Unattached" ]]; then 
		echo "IMAGE INACTIVE"; 
		counter=10
		#wait for disk to be created before attaching, otherwise skip after 30s
		while [[ `az disk show -n disk${regionname}$line_next1 -g $resourcegroup | jq -r '"\(.diskState)"'` != "Unattached" ]] && [ $counter -ge 1 ]; do sleep 3; echo "WAITING DISK CREATION";if [ $counter -eq 1 ]; then skip_mount=1; echo "IMAGE DEPRECATED NR $line_next1"; echo "Skipping image $line_next1"; fi; ((counter-=1)); done
		diskId=$(az disk show -g $resourcegroup -n disk${regionname}$line_next1 --query 'id' -o tsv)
